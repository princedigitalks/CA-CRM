...
export default function DashboardPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto">
      <div className="mb-10">
        <p className="text-gray-400 text-sm font-medium mb-1">Vault / <span className="text-blue-600">Overview</span></p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white rounded-[40px] border border-gray-100 shadow-sm p-8">
          ...
          <div className="space-y-4">
            {recent.length === 0 && (
              <p className="text-gray-400 text-sm text-center py-8">No clients yet. <Link href="/clients" className="text-blue-600 font-bold">Add one</Link></p>
            )}
            {recent.map((client, i) => (
              <motion.div
                key={client.id}
              ...
              >
                <Link href={`/clients/${client.id}`} className="flex items-center gap-4 p-4 rounded-2xl hover:bg-gray-50 transition-colors group">
                  ...
                  <div className="flex items-center gap-4 text-xs text-gray-400">
                    ...
                    <span className="flex items-center gap-1"><FileText size={13} /> {client.documents.length}</span>
                    <span className={`px-2 py-1 rounded-full font-bold text-[10px] ${client.status === 'ACTIVE' ? 'bg-blue-50 text-blue-600' : 'bg-gray-100 text-gray-500'}`}>
                      {client.status}
                    </span>
                  </div>
                </Link>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-[40px] border border-gray-100 shadow-sm p-8">
            <h3 className="font-bold text-gray-900 mb-4">Active Clients</h3>
            <div className="space-y-3">
